...
def propose_for_document(state: AgentState) -> dict[str, Any]:
    ...
    for filename, text in state["file_texts"].items():
        ...
        # Rejections recorded on a prior pass through the retry loop (see apply_review_answer
        # and check_and_correct_proposals) - fed back into the prompts below so the LLM doesn't
        # propose the same already-rejected entities again, and carried forward into the fresh
        # proposal dict built at the end of this loop so they keep accumulating across retries.
        ...
        # TODO Currently, LLM Summary is set to only really large documents as it made the classification worse in test runs. We keep the code here for future improvement.
        ...
        proposals[filename] = {
            ...
            "confirmed": False,
            "needs_retry": False,
            # Carried forward unchanged from the previous attempt (if any) - only
            # apply_review_answer/check_and_correct_proposals add to these, this function
            # never rejects anything itself, it just keeps the history from getting wiped.
            "rejected_existing_tags": rejected_existing_tags,
            "rejected_existing_correspondent": rejected_existing_correspondent,
            "rejected_existing_document_type": rejected_existing_document_type,
            "rejected_new_tags": rejected_new_tags,
